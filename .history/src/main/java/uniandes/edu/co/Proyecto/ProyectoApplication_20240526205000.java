...
public class ProyectoApplication implements CommandLineRunner{
	...
	CommandLineRunner runner2(UsuarioRepository usuarioRepository, CuentaRepository cuentaRepository, OperacionRepository operacionRepository, MongoTemplate mongoTemplate){
		return args -> {
			...
			Usuario usuario = new Usuario(10, "Activa", "Gerente General", cuentas2);
			...
			if(usuarios.isEmpty()){ 	
				System.out.println("Creando cuenta del usuario "+10);
				usuarioRepository.insert(usuario);
			} else{
				System.out.println("No se ha encontrado el usuario, poner id distinto a "+10);
			}
			...
			if(cuentas.isEmpty()){ 	
				...
				usuarioRepository.aniadirCuentaAUsuario(10, cuenta.getTipo(), cuenta.getEstado(), 0, cuenta.getNumero());
			} else{
