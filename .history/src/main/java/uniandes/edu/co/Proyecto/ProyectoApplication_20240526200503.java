...
public class ProyectoApplication {
	...
	CommandLineRunner runner(UsuarioRepository usuarioRepository, CuentaRepository cuentaRepository, OperacionRepository operacionRepository, MongoTemplate mongoTemplate){
		return args -> {
			...
			if(usuarios.isEmpty()){ 	
				System.out.println("Creando cuenta del usuario "+usuario);
			} else{
				System.out.println(usuario+" no se ha encontrado, poner id distinto a "+usuario.getId());
			}

			Query query2 = new Query();
			query.addCriteria(Criteria.where("id").is(cuenta.getId()));

			List<Cuentas> cuentas = mongoTemplate.find(query, Usuario.class);
		};
	}
